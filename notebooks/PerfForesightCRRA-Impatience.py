...
# Now calculate the "sustainable" level of consumption that leaves wealth untouched
# and plot it against the perfect foresight solution
# SOLUTIONS

cSustainable = ((Rfree-1)/Rfree)*(m_range-1+1/(1-PermGroFac[0]/Rfree)) # For any given level of m, the level of c that would leave wealth unchanged

plt.figure(figsize=(9,6)) # set the figure size
plt.xlabel('Market resources m') # x axis label
plt.ylabel('Consumption c') # y axis label

plt.plot(m_range, cSustainable  , 'g', label='Sustainable c') # Add sustainable c line
plt.plot(m_range, cHARK, 'b', label='c Function')
plt.legend()
plt.show() # show the plot
...
# Compute the values of the impatience conditions under default parameter values -- SOLUTIONS
...
# Now plot the consumption functions for values of rho above the default value -- SOLUTIONS

CRRA_min = CRRA
CRRA_max = 100
CRRAArray = np.exp(np.linspace(np.log(CRRA_min),np.log(CRRA_max), numPoints, endpoint=True))
CRRAList = CRRAArray.tolist() # Make growth factors a list
...
for i in range(len(CRRAList)):
    ...
    PFagent.solve()
    cHARK = PFagent.solution[0].cFunc(m_range)
    plt.plot(m_range, cHARK, label='Consumption Function, $\\rho=$ '+str(CRRAList[i]))

#PFagent.CRRA = Params.CRRA      # return CRRA to default value
#PFagent.solve() # return solution to its default value
plt.xlabel('Market resources m') # x axis label
plt.ylabel('Consumption c')      # y axis label
plt.legend()                     # show legend
plt.show()                       # plot chart
