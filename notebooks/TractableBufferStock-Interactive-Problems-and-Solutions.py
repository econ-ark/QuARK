...
# Coding the new TBS plot
interact(makeTBSplot,
         DiscFac = DiscFac_widget2,
         CRRA = CRRA_widget,
         # We can fix a parameter using the fixed() operator
         Rfree      = fixed(TBS_dictionary['Rfree']),
#         Rfree = Rfree_widget,   # This is the line which, when uncommented, would make Rfree a slider
...
# %% [markdown]
# The dotted line in the above phase diagram of market resources $m_{t}$ and consumption $c_{t}$ is 'the sustainable consumption locus' ($\Delta m^{e} = 0$). For any given level of market resources $m^{e}_{t}$, consuming less than the 'sustainable' level of consumption causes wealth for the employed agent's next period to rise. This mechanical relationship between consumption and market resources is not altered by the discount factor, and so remains constant for different levels of the discount factor $\beta$.
#
# The blue line is the employed agents' consumption function given its market resources $c^{e}(m)$, formed by $\Delta m^{e} = 0$ ('the sustainable consumption locus') and $\Delta c^{e} = 0$ (the locus in which the employed agents' consumption remains unchanged). A greater discount factor (or more weight towards future consumption conditional on employment) leads to greater saving and thus greater market resources $m_{t}$ in the next period.
#
# The optimal level of market resources and consumption ($m_{t}, c_{t}$) is at the intersection of the sustainable consumption locus $\Delta m^{e} = 0$ and the $\Delta c^{e} = 0$ locus, and is therefore also on the intersection o fthe sustainable consumption locus $\Delta m^{e} = 0$ and the consumption function $c^{e}(m)$.
